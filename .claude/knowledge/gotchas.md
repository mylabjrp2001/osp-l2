...
## กฎรายงานที่ใช้ร่วมกัน อยู่ที่เดียวแล้ว (18 ก.ย. 2026)

เดิม 3 อย่างนี้ถูกก๊อปไว้ไฟล์ละชุด (เทียบแล้วโค้ดเหมือนกันทุกตัวอักษร) แก้หน้าเดียวตัวเลขจะไม่ตรงกัน
ตอนนี้ย้ายมาไว้ที่เดียว — **ห้ามประกาศซ้ำในหน้าใดหน้าหนึ่งอีก ให้ import ไปใช้**

| ของ | อยู่ที่ | ใครใช้ |
|---|---|---|
| `SLA_PRIORITIES` = Critical + Major | `src/theme.js` | Page05 · SlaPerTeamPage (หน้า 9–10) · TotalJobOverviewPage (หน้า 11–18) |
| `lastSixMonths(end)` | `src/utils.js` | Page05 · Page08 · SlaPerTeamPage |
| `prevMonthRange(end)` | `src/utils.js` | Page21 · Page22 · AvgDurationPage |

สิ่งที่ **ตั้งใจให้ต่างกันรายหน้า** คือ argument `skip` ของ `applyFilters(records, f, skip)` เช่น
หน้า 5 ข้าม date+zone+priority เพราะใช้หน้าต่าง 6 เดือนตายตัว · หน้า 9–10 ข้าม team เพราะแยกเส้นรายทีมเอง
อันนั้นไม่ใช่โค้ดซ้ำ อย่าไปรวบ
...
## stop.bat — ปิดโปรแกรมเมื่อหาหน้าต่างไม่เจอ

`stop.bat` เรียก `start-windows.ps1 -Stop` → หา process ที่ listen พอร์ต 8000 และ 5173
แล้ว `taskkill /T /F` **เฉพาะชื่อ python / pythonw / node** ถ้าเป็นโปรแกรมอื่นจะพิมพ์ชื่อบอกแล้วไม่แตะ
(อย่าเอาเงื่อนไขนี้ออก ไม่งั้นมันจะไปปิดโปรแกรมอื่นของผู้ใช้ที่บังเอิญใช้พอร์ตเดียวกัน)

กด `start.bat` ซ้ำตอนที่รันอยู่ = ถามว่าจะปิดเลยไหม (พิมพ์ S) ไม่ก็แค่เปิดเบราว์เซอร์ให้ใหม่
